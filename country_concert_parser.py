...
class CountryConcertParser:
    ...
    def fetch_page(self, url: str) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            response = self.session.get(url, timeout=self.REQUEST_TIMEOUT, verify=False, headers=headers, allow_redirects=True)
    ...
    def parse_page(self, soup: BeautifulSoup) -> Tuple[List[Dict], List[Dict]]:
        """Parse all concerts from a single page, return (all_concerts, filtered_concerts)"""
        all_concerts = []
        filtered_concerts = []
        ...
        for event_div in event_divs:
            ...
            if event_details['event_name']:
                all_concerts.append(event_details)
    ...
    def parse_all_pages(self, on_page_complete=None):
        ...
        while True:
            ...
            print(f"  Found {len(all_concerts)} concerts on page {page_num} ({len(filtered_concerts)} matched Last.fm artists)")
...
def main():
    ...
    if not args.no_filter:
        ...
        lastfm_artists, recent_artists, artist_playcounts = fetch_lastfm_artists(
            lastfm_api_key,
            user=lastfm_user,
