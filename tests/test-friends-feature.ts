...
async function testSendFriendRequest() {
  ...
  if (savedFriendship?.status !== 'PENDING') {
...
async function testFriendLimit() {
  ...
  const friendCount = await prisma.friendship.count({
    where: {
      OR: [
        { userId: user9.id, status: 'ACCEPTED' },
        { friendId: user9.id, status: 'ACCEPTED' }
      ]
    }
  });
...
async function testAlphabeticalSorting() {
  ...
  const friendships = await prisma.friendship.findMany({
    where: {
      OR: [
        { userId: user1.id, status: 'ACCEPTED' },
        { friendId: user1.id, status: 'ACCEPTED' }
      ]
    },
    include: {
